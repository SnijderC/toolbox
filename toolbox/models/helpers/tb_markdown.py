...
class ToolboxMD(markdown.Markdown):
    ...
    def __init__(
            self,
            extensions=std_extensions,
            cached_wordlist=True,
            output_format='html5'):

        # pattern for  filter_abbr_anchored()
        self.abbr_anchor_pat = re.compile(
            r"(<a(?![a-z]).*?>.*?)<abbr.*>(.*)</abbr>(.*?</a>)",
            flags=re.MULTILINE
        )
        # pattern for process_inline_images()
        self.inline_img_pat = re.compile(
            r'(?<!\\)\!(?:\[.*\])?\[([a-zA-Z0-9\-\+\_\.\ ]{4,200})\]')
        self.cached_wordlist = cached_wordlist
        self.wordlist = None

        super(ToolboxMD, self).__init__(
    ...
    def convert(self, str_markdown, topics=[]):
        '''
            ...
            Functions tasks:
            - Convert MD to HTML
            - Find all terms in the terms list and generate the MD code to make
              popovers with explanations.
            - Find all popovers that would have been placed inside of url's and
              filter them out of them (HTML):

        '''
        self.reset()
        if self.cached_wordlist:
            if self.wordlist is None or self.topics != topics:
                self.generate_wordlist(topics)
        else:
            self.generate_wordlist(topics)

        return self.filter_abbr_anchored(super(ToolboxMD, self).convert(
            str_markdown+self.wordlist
        ).encode("utf-8"))
    ...
    def process_inline_images(self, str_md):
        """
            ...
            ID's can contain [ a-Z 0-9 - + _ . ] and space, min 4, max 200
            tekens.
    ...
    def filter_abbr_anchored(self, str_html):
        '''
            ...
            Markdown automatically generates <abbr> tags for wordlists
            (add-on).

            In the toolbox we now use popovers for <abbr> tags.
            It looks pretty bad when these show up wrapped inside links..

            Preventing it from happening seems pretty hard so to hack around
            this issue I remove the <abbr...><abbr> with regex magic.
            ...
            1. Grabs the start of the anchor and it's contents up to the start
               of the <abbr> tag.
            x  <abbr.*> should be discarded
            2. Captures anything inside the <abbr> tag (that should not be
               lost).
            x  </abbr> is the end of the <abbr> tag and should be discarded
            3. Is the last bit of text before the anchor is closed, including
               the anchor's closing tag.
