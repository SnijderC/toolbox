# -*- coding: utf-8 -*-
from toolbox.models import Category, ParentCategories
from toolbox.models import Platform, Formfactor, Playlist
from settings import slugs as sluglist
import copy
...
class Navigation(object):
    ...
    def __init__(self, slugs):
        """
            ...
            sure some settings are only singular, others accumulate, some
            replace others.
        """
        ...
        for val in sluglist:
            self.sluglistrev[sluglist[val]['slug']] = copy.deepcopy(
                sluglist[val])
            self.sluglistrev[sluglist[val]['slug']]['slug'] = val

        # Retrieve the categories and other category types from the DB and make
        # dicts out of them also make some URL's to each one.
        self.categories, self.categorieslinks = self.getfilteritems(
            Category, "categories")
        self.platforms, self.platformslinks = self.getfilteritems(
            Platform, "platforms")
        self.formfactors, self.formfactorslinks = self.getfilteritems(
        ...
        if len(
            set(('formfactors', 'categories', 'platforms')) &
            set(self.filters)
        ) > 0:
    ...
    def set_navitems(self):
        ...
        filters.append(
            filter_dropdown('platforms', 'tb-platforms', 'Alle apparaten'))
        ...
        for formfactor in objformfactor.all():
            ...
            if ('formfactors' in self.filters.keys() and
                    self.filters['formfactors'][0] == formfactor.slug):
            ...
            for platform in formfactor.platforms.all():
                platformobj = {
                    'name': platform.name,
                    'icon': platform.icon,
                    'href': self.makelink("formfactor/%s/platform/%s" % (
                        formfactor.slug,
                        platform.slug
                    )),
                }
                ...
                if ('platforms' in self.filters.keys() and
                        self.filters['platforms'][0] == platform.slug):
                    str_platform_name = platform.name
                    str_platform_icon = platform.icon

            if (str_formfactor_name is not None and
                    str_platform_name is not None):
                filters[0][
                    'name'] = "%s + %s" % (
                        str_formfactor_name,
                        str_platform_name
            ...
            elif str_formfactor_name is not None:
                filters[0]['name'] = str_formfactor_name
                filters[0]['icon'] = str_formfactor_icon
            elif str_platform_name is not None:
                filters[0]['name'] = str_platform_name
                filters[0]['icon'] = str_platform_icon

        if str_formfactor_name is not None or str_platform_name is not None:
            filterobj.insert(0, {
                'name': "Alle apparaten",
        ...
        filters.append(
            filter_dropdown('categories', 'tb-tags', 'Alle categorieën'))
        filterobj = filters[-1]['children']
        ...
        for parentcategory in parentcategories.all():
            ...
            for category in parentcategory.categories.all():
                ...
                if ('categories' in self.filters.keys() and
                        self.filters['categories'][0] == category.slug):
    ...
    def getfilteritems(self, model, slug):
        ...
        for item in obj_items:
            ...
            directlinks[item.slug] = self.makelink(
                "%s/%s" % (revslug, item.slug))
            ...
            if slug not in removeslugs:
                ...
                # New links will forget item_arg which is a single item..
                    ...
                    """
                        ...
                        this is deemed to complicated behaviour for average
                        users always direct back to overview unless
                        specifically chosen..
                    """
                    # Previously users were led to /overzicht/[whatever]
                    # filters if it was not specifically mentioned in the
                    # slugstr, now it also preserves the specific index page
                    # (/tools or /advies). Not sure what is considered most
                    # user friendly but I think this is.
                    # if linkslugs[slug]:
                    #    # specifically chosen..
    ...
    def process_slugs(self, slugs=""):
        """
            ...
            There are 2 types of slugs, those that need another slug as a
            keyword and those that do not need one. Loop through the slugs and
            return the sets or single slugs.

            Some slugs are of the "single" type, those can be queried with or
            without a keyword. There can be only one specified. This is not
            checked but only the last that is mentioned will be valid.

            1. Split the string into all the slugs found in the slugs.
            2. Temporarily store the "key" and "value" in the arr_filters
               object. Check whether the slug belongs in the "key" or "value"
               placeholder. If the string is mentioned in settings.views.slugs
               it's a key. These values SHOULD not be in the slugs of the
               database but are not currently validated..
            3. If a "key" slug is found, store it IF:
                a. The next slug is not a key (it's a value), then also store
                   the value.
                b. The next slug is also a key but the current slug is a
                   "single" key.
        ...
        if slugs:
            ...
            for i, slug in arr_slugs_enum:
                if slug in sluglist.keys():
                    ...
                    if (i < len(arr_slugs)-1 and
                            arr_slugs[i+1] not in sluglist.keys()):
                        ...
                        # skip the next "slug" since it's not a slug, it's the
                        # value..
                        [next(arr_slugs_enum)]
