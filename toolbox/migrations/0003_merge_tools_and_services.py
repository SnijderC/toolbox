...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'App'
        db.create_table(u'toolbox_app', (
            (u'tool_ptr', self.gf('django.db.models.fields.related.OneToOneField')(to=orm['toolbox.Tool'], unique=True, primary_key=True)),
        ...
        db.send_create_signal('toolbox', ['App'])

        # FIRST WE SHOULD MIGRATE THE APPSTORE DATA..
        if not db.dry_run:
            cursor = connection.cursor()
            
            cursor.execute('''INSERT INTO toolbox_app (playstore,appstore,marketplace, tool_ptr_id)
                              SELECT playstore, appstore, marketplace, id FROM toolbox_tool''')
                
        # Deleting field 'Tool.marketplace'
        db.delete_column(u'toolbox_tool', 'marketplace')

        # Deleting field 'Tool.playstore'
        db.delete_column(u'toolbox_tool', 'playstore')

        # Deleting field 'Tool.appstore'
        db.delete_column(u'toolbox_tool', 'appstore')


    def backwards(self, orm):
        

        # Adding field 'Tool.marketplace'
        db.add_column(u'toolbox_tool', 'marketplace',
                      self.gf('django.db.models.fields.CharField')(default='', max_length=150, blank=True),
                      keep_default=False)

        # Adding field 'Tool.playstore'
        db.add_column(u'toolbox_tool', 'playstore',
                      self.gf('django.db.models.fields.CharField')(default='', max_length=150, blank=True),
                      keep_default=False)

        # Adding field 'Tool.appstore'
        db.add_column(u'toolbox_tool', 'appstore',
                      self.gf('django.db.models.fields.CharField')(default='', max_length=150, blank=True),
                      keep_default=False)

        if not db.dry_run:
            for tool in orm.Tool.objects.all():
                tool.playstore = tool.app['playstore']
                tool.marketplace = tool.app['marketplace']
                tool.appstore = tool.app['appstore']
            
        # Deleting model 'App'
        db.delete_table(u'toolbox_app')
