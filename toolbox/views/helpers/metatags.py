...
def set_metatags(metatags, article):
    ...
    try:
        ...
        metatags['permalink'] = meta_templates['permalink'] % \
            "/%s/%s/" % (article.url_slug(), article.slug)

        try:
            if article.has_image():
                metatags['imagelink'] = meta_templates['permalink'] % \
                    get_thumbnailer(article.image)['social'].url
            else:
        ...
        if hasattr(article, 'i_am'):
            metatags['tweet_article'] = tweet_templates[
                article.i_am()]  # % article.title
