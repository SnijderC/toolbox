...
\* *One thing that can be said about another backend is that the search functionality will not work. You will need to implement a custom solution as this feature uses MySQL's.*
...
## Installation instructions
 ...
 3. Make appropriate changes to the `settings/production.py` file, at a minimum supply a database backend, username and password.
    Find these settings in the `settings/production.py`
    Note: *For a development setup you need to change the `develop.py` file instead. Settings in `develop.py` overwrite settings in `production.py`.*
...
- Grunt is used to compile LESS files and combine the necessary Javascripts and compress everything into a single CSS and a single JS file. The Gruntfile.js is already set to go, workflows as follows:
 - `grunt install` to install all Grunt's dependencies in `package.json`.
 - `grunt server`
