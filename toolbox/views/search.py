...
SELECT
    ...
    (IF(`%(table)s`.`%(title)s` LIKE %%s,2,1) * MATCH (%(matches)s) AGAINST
        ("%%s" IN BOOLEAN MODE)) AS `relevance`
FROM
    `%(table)s`
WHERE
    %(check_published)s
...
'''
    try:
        ...
        # This array determines the models and fields to be searched for the
        # search function.
        ...
        for model in models:
            ...
            if model['checkpublished']:
                model[
                    'check_published'] = '`%s`.`published` = TRUE AND\n' % \
                        tablename
            ...
            for res in model['model'].objects.raw(
                strquery,
                [like, query, query]
            ):
